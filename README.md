...
### 8. Virtual DOM 구현
JSX는 `h()` 호출로 변환되고, `createElement()`가 `type`, `props`, `children`, `key`를 가진 vnode 객체를 생성한다.  
문자열과 숫자는 텍스트용 vnode로 변환하여 일반 노드와 같은 방식으로 처리한다.

또한 함수형 자식 컴포넌트는 `resolveVNode()`에서 실행되어 최종 host vnode 트리로 평탄화된다.  
이 과정을 거친 뒤에만 실제 DOM 생성과 diff/patch가 수행된다.

테스트 페이지에서 Virtual DOM은 화면 전체를 설명하는 객체 트리 역할을 하며, 다음 UI 영역이 모두 vnode 트리로 생성된다.
...
### 13. 테스트 페이지에서 최소 구현 대상이 드러나는 위치
